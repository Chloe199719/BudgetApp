...
use crate::{ helpers::spawn_app, users::login::LoginUser, categories::create_category_in_db };
...
async fn test_edit_category_one_field_name_success(pool: PgPool) {
    ...
    let edit_category_response = app.api_client
        .put(&format!("{}/categories/edit/{}", app.address, category.category_id))
    ...
    assert_eq!(edit_category_response_body.category_name, edit_category_body.name.unwrap());
}
...
async fn test_edit_category_one_field_description_success(pool: PgPool) {
    ...
    assert_eq!(edit_category_response_body.description, edit_category_body.description.unwrap());
}
