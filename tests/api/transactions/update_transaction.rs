...
async fn test_update_transaction_success_change_amount(pool: PgPool) {
    ...
    assert_eq!(response_body.transaction_type, "DEPOSIT".to_string());
    assert_eq!(
        response_body.description,
        create_transaction_body.description
    );
    assert_eq!(response_body.user_id, login_response_body.id);
    assert_eq!(
        response_body.date.date_naive(),
        create_transaction_body.transaction_date.date_naive()
    );
    assert_eq!(
...
async fn test_update_transaction_success_change_currency(pool: PgPool) {
    ...
    assert_eq!(response_body.amount, create_transaction_body.amount);
    assert_eq!(response_body.user_id, login_response_body.id);
    assert_eq!(
        response_body.date.date_naive(),
        create_transaction_body.transaction_date.date_naive()
...
async fn test_update_transaction_success_change_description(pool: PgPool) {
    ...
    let update_transaction_body = UpdateTransaction {
        amount: None,
        currency: None,
        description: Some(Sentence(1..2).fake()),
    };
    ...
    assert_eq!(
        response_body.description,
        update_transaction_body.description.unwrap()
    );
    assert_eq!(response_body.amount, create_transaction_body.amount);
    assert_eq!(response_body.user_id, login_response_body.id);
    assert_eq!(
