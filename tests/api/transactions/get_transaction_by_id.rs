...
#[sqlx::test]
async fn get_transaction_by_id_success(pool: PgPool) {
    ...
    assert!(get_transaction_by_id_first.status().is_success());
    let get_transactions_1_response_body = get_transaction_by_id_first
        .json::<TransactionOutcomeWithReceipt>()
        .await
        .expect("Failed to parse get all transactions response");

    assert_eq!(
        get_transactions_1_response_body.transaction_type,
        "DEPOSIT".to_string()
    );
    assert_eq!(
        get_transactions_1_response_body.description,
        create_transaction_body.description
    );
    assert_eq!(
        get_transactions_1_response_body.amount,
        create_transaction_body.amount
    );
    assert_eq!(
        get_transactions_1_response_body.user_id,
        login_response_body.id
    );
    assert_eq!(
        get_transactions_1_response_body.date.date_naive(),
        create_transaction_body.transaction_date.date_naive()
    );
    assert_eq!(
        get_transactions_1_response_body.currency.to_string(),
        create_transaction_body.currency.to_string()
    );
    assert_eq!(
        get_transactions_1_response_body.category_id,
        create_transaction_body.category_id
    );
    let get_transaction_by_id_two = app
        .api_client
        .get(&format!(
            "{}/transactions/get_transaction_by_id/{}",
            app.address, create_transaction_response_body2.transaction_id
        ))
        .send()
        .await
        .expect("Failed to execute request.");

    assert!(get_transaction_by_id_two.status().is_success());
    let get_transactions_2_response_body = get_transaction_by_id_two
        .json::<TransactionOutcomeWithReceipt>()
        .await
        .expect("Failed to parse get all transactions response");

    assert_eq!(
        get_transactions_2_response_body.transaction_type,
        "WITHDRAWAL".to_string()
    );
    assert_eq!(
        get_transactions_2_response_body.description,
        create_transaction_body2.description
    );
    assert_eq!(
        get_transactions_2_response_body.amount,
        create_transaction_body2.amount
    );
    assert_eq!(
        get_transactions_2_response_body.user_id,
        login_response_body.id
    );
    assert_eq!(
        get_transactions_2_response_body.date.date_naive(),
        create_transaction_body2.transaction_date.date_naive()
    );
    assert_eq!(
        get_transactions_2_response_body.currency.to_string(),
        create_transaction_body2.currency.to_string()
    );
    assert_eq!(
        get_transactions_2_response_body.category_id,
        create_transaction_body2.category_id
    );
}

#[sqlx::test]
async fn get_transaction_by_id_not_found(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    //Act - Part 1 - Login
    let login_body = LoginUser {
        email: app.test_user.email.clone(),
