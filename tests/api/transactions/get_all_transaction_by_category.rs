...
#[sqlx::test]
async fn get_all_transactions_success_by_category_default(pool: PgPool) {
    ...
    let get_all_transactions_response = app
        .api_client
        .get(&format!(
            "{}/transactions/get_all_transactions_by_category",
            app.address
        ))
        .send()
        .await
...
#[sqlx::test]
async fn get_all_transactions_success_by_category_id(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    //Act - Part 1 - Login
    let login_body = LoginUser {
        email: app.test_user.email.clone(),
