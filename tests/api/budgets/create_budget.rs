...
#[sqlx::test]
async fn test_create_budget_success(pool: PgPool) {
...
#[sqlx::test]
async fn test_create_budget_success_recurring(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
...
#[sqlx::test]
async fn test_create_budget_error_category_dosent_exist(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    ...
    let create_budget_response = app
        .api_client
        .post(&format!("{}/budgets/create/{}", app.address, 100))
        .json(&create_budget_body)
        .send()
        .await
        .expect("Failed to execute request.");

    assert!(create_budget_response.status().is_client_error());
}

#[sqlx::test]
async fn test_create_budget_error_budget_already_exists_in_category(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    ...
    let create_budget_response2 = app
        .api_client
        .post(&format!("{}/budgets/create/{}", app.address, 1))
        .json(&create_budget_body)
        .send()
    ...
    assert!(create_budget_response2.status().is_client_error());
}
