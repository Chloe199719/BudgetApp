...
#[sqlx::test]
async fn test_change_budget_dates(pool: PgPool) {
    ...
    let change_budget_amount_body = ChangeBudgetDates {
        start_date: chrono::Utc::now() + chrono::Duration::days(1),
        end_date: chrono::Utc::now() + chrono::Duration::days(31),
    };

    let change_budget_amount_response = app
        .api_client
        .patch(&format!("{}/budgets/change_date/{}", app.address, 1))
    ...
    assert_eq!(
        change_budget_amount_body.start_date.timestamp_micros(),
    ...
    assert_eq!(
        change_budget_amount_body.end_date.timestamp_micros(),
        get_category_response_body
            .end_date
            .unwrap()
            .timestamp_micros()
...
#[sqlx::test]
async fn test_change_budget_dates_error(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    ...
    let change_budget_amount_response = app
        .api_client
        .patch(&format!("{}/budgets/change_date/{}", app.address, 100))
        .json(&change_budget_amount_body)
        .send()
        .await
        .expect("Failed to execute request.");
