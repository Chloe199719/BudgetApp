...
pub struct ChangeBudgetRecurring {
    recurring: bool,
}
...
async fn test_change_budget_dates(pool: PgPool) {
    ...
    let change_budget_amount_body = ChangeBudgetRecurring { recurring: false };

    let change_budget_amount_response = app
        .api_client
        .patch(&format!("{}/budgets/change_recurring/{}", app.address, 1))
    ...
    assert_eq!(
        change_budget_amount_body.recurring,
...
#[sqlx::test]
async fn test_change_budget_recurring_error(pool: PgPool) {
    let app = spawn_app(pool.clone()).await;
    ...
    //Act - Part 3 - Change budget amount
    let change_budget_amount_body = ChangeBudgetRecurring { recurring: true };

    let change_budget_amount_response = app
        .api_client
        .patch(&format!("{}/budgets/change_recurring/{}", app.address, 100))
        .json(&change_budget_amount_body)
        .send()
        .await
        .expect("Failed to execute request.");
