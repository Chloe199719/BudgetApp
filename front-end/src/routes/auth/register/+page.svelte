<script lang="ts">
    ...
    let email = "" , password = "", unique_name = "", display_name = "", confirmPassword = "", errors:Array<CustomError>= [], fieldsError = {email: "", password:"", unique_name:"", display_name:"", confirmPassword:""}, isValid= false;
    
    const handleRegister = async ()=> {
        isValid = true;
        if(!isValidEmail(email)){
            isValid = false;
            fieldsError.email = "Please enter a valid email address";
        } else {
            fieldsError.email = "";
        }
        if(!isValidPasswordMedium(password)){
            isValid = false;
            fieldsError.password = "Password must be at least 8 characters long";
        } else {
            fieldsError.password = "";
        }
        if(!isValidPasswordStrong(password)){
            isValid = false;
            fieldsError.password = "Password must contain at least one uppercase letter, one lowercase letter, one number and one special character";
        } else {
            fieldsError.password = "";
        }
...
<div class="flex items-center justify-center h-[60vh]">
    <form
    class="w-11/12 md:w2/3 lg:w1/3 rounded-xl flex flex-col items-center bg-slate-800 py-4"
    ...
    <div class="w-3/4 mb-2">
        <input type="email" name="email" id="email" bind:value={email} placeholder="Email Address" required class="w-full text-sky-500 placeholder:text-slate-600 border-none focus:ring-0 bg-main-color focus:outline-none py-2 px-3 rounded">
    ...
    <div class="w-3/4 mb-2">
        <input type="text" name="display_name" id="display_name" class="w-full text-sky-500 placeholder:text-slate-600 border-none focus:ring-0 bg-main-color focus:outline-none py-2 px-3 rounded" bind:value={display_name} placeholder="Display_name" required>
    </div>
    <div class="w-3/4 mb-2">
        <input type="text" name="unique_name" id="unique_name" class="w-full text-sky-500 placeholder:text-slate-600 border-none focus:ring-0 bg-main-color focus:outline-none py-2 px-3 rounded" bind:value={unique_name} placeholder="Unique_name" required>
    </div>
    <div class="w-3/4 mb-2">
        <input
            type="password"
