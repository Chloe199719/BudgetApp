...
use crate::{
    utils::{ users::get_active_user_from_db, constant::BACK_END_TARGET },
    types::{ UserVisible, general::ErrorResponse },
};

use super::logout::session_user_id;

#[tracing::instrument(name = "Accessing retrieving current user endpoint.", skip(pool, session))]
...
pub async fn get_current_user(pool: Data<PgPool>, session: actix_session::Session) -> HttpResponse {
    match session_user_id(&session).await {
        Ok(id) => {
            match get_active_user_from_db(Some(&pool), None, Some(id), None).await {
                Ok(user) => {
                ...
                Err(e) => {
                    ...
                    let error_message = ErrorResponse { error: "User was not found".to_string() };
