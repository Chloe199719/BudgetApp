...
use super::auth::tokens::issue_confirmation_token_pasetors;
...
pub async fn send_email(
    ...
    let email = lettre::Message
        ...
        .to(format!("{} <{}>", recipient_display_name, recipient_email).parse().unwrap())
        .subject(subject)
        .multipart(
    ...
    let mailer: lettre::AsyncSmtpTransport<lettre::Tokio1Executor> = lettre::AsyncSmtpTransport::<lettre::Tokio1Executor>
        ::relay(&settings.email.host)
        .unwrap()
        .credentials(creds)
        .build();
...
pub async fn send_multipart_email(
    ...
    let text =
        ...
        "#, confirmation_link);
    tokio::spawn(
        send_email(None, recipient_email, recipient_display_name, subject, html_text, text)
    );
    Ok(())
}
