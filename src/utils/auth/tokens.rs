...
use crate::types::tokens::ConfirmationToken;

/// Store the session key perfix as const so ti can't be typo'd anywhere it's used
...
pub async fn issue_confirmation_token_pasetors(
    ...
    let session_key: String = {
        ...
        OsRng::fill_bytes(&mut OsRng, &mut buff);
        hex::encode(buff)
    };
    ...
    redis_connection.set(redis_key.clone(), String::new()).await.map_err(|e| {
        tracing::event!(target:"discord_backend", tracing::Level::ERROR,  "RedisError (set): {}", e);
        e
    })?;
    ...
    redis_connection
        .expire(redis_key.clone(), time_to_live.num_seconds().try_into().unwrap()).await
        .map_err(|e| {
            tracing::event!(target:"discord_backend", tracing::Level::ERROR,  "RedisError (expiry): {}", e);
            e
        })?;
...
pub async fn verify_confirmation_token_pasetor(
    ...
    let untrusted_token = UntrustedToken::<Local, V4>
        ::try_from(&token)
        .map_err(|e| { format!("TokenValidation: {}", e) })?;

    let trusted_token = local
        ::decrypt(
            &sk,
            &untrusted_token,
            &validation_rules,
    ...
    match serde_json::from_value::<String>(uid) {
        Ok(uuid_string) =>
            match uuid::Uuid::parse_str(&uuid_string) {
                Ok(user_uuid) => {
                    let sss_key = serde_json
                        ::to_value(claims.get_claim("session_key").unwrap())
                        .unwrap();
                    ...
                    if
                        redis_connection
                            .get::<_, Option<String>>(redis_key.clone()).await
                            .map_err(|e| format!("{}", e))?
                            .is_none()
                    {
                        return Err("Token has been used or expired.".to_string());
                    }
                    redis_connection.del(redis_key.clone()).await.map_err(|e| format!("{}", e))?;
